...
window.addEventListener("error", (event) => {
    ...
    if (shouldWarnAboutProxyError(url)) {
        showProxyWarning(
            "The browser backend is being rate limited or returned an error. Please wait a few seconds and try again."
        );
    }
}, true);
...
window.addEventListener("unhandledrejection", (event) => {
    ...
    if (reason && reason.includes("Failed to fetch dynamically imported module")) {
        showProxyWarning(
            "The browser failed to fetch a module because the upstream server responded with an error. Please reload when the browser recovers."
        );
    }
...
document.addEventListener("DOMContentLoaded", () => {
	...
	function shouldHardRepairProxyRuntime(value) {
		const text = normalizeMuxError(value).toLowerCase();
		if (!text) return false;
		return (
			text.includes("there are no bare clients") ||
	...
	async function clearProxyRuntimeData() {
		// Preserve user selections; auto-repair should not silently change settings.
		...
		try {
			if ("caches" in window) {
				...
				await Promise.allSettled(
					keys
						.filter((k) => /uv|bare|mux|scram|epoxy|eclipse/i.test(k))
		...
		try {
			...
			if ("indexedDB" in window && typeof indexedDB.databases === "function") {
				try {
					...
					dbs.forEach((db) => {
						const n = db?.name || "";
						if (/bare|mux|uv|scram|epoxy|eclipse|idbmap/i.test(n)) names.add(n);
					});
				} catch {}
			}
			await Promise.allSettled(
	...
	async function hardRepairProxyRuntime(reason) {
		const guardKey = "proxy_runtime_hard_repair_done";
		try {
			if (sessionStorage.getItem(guardKey) === "1") return;
			sessionStorage.setItem(guardKey, "1");
		} catch {}
		console.warn("Running automatic proxy runtime repair:", reason);
	...
	function handleMuxError(value, label) {
		...
		if (shouldHardRepairProxyRuntime(value)) {
			hardRepairProxyRuntime(label);
		}
	}

	let muxWarnBurstCount = 0;
	let muxWarnBurstStartedAt = 0;
	const origConsoleWarn = console.warn.bind(console);
	console.warn = function patchedConsoleWarn(...args) {
		try {
			const msg = args.map((v) => normalizeMuxError(v)).join(" ").toLowerCase();
			if (msg.includes("failed to get a bare-mux sharedworker messageport")) {
				const now = Date.now();
				if (!muxWarnBurstStartedAt || now - muxWarnBurstStartedAt > 12000) {
					muxWarnBurstStartedAt = now;
					muxWarnBurstCount = 0;
				}
				muxWarnBurstCount += 1;
				if (muxWarnBurstCount >= 6) {
					hardRepairProxyRuntime("console.warn burst");
				}
			}
		} catch {}
	...
	async function sjEncode(url) {
		let encodedUrl = null;
		try {
			encodedUrl = scramjet.encodeUrl(url);
		} catch {
			encodedUrl = null;
		}
		if (!encodedUrl) {
			encodedUrl = encodeScramjetRoute(url);
		}
		logHistory(url);
		safeStore("url", encodedUrl);
        createTab(encodedUrl);
	...
	function shouldForceScramjetForUrl(inputUrl) {
		...
		// Best-effort parse: accept bare domains (example.com) as https://example.com.
		let parsed = null;
		try {
			parsed = new URL(raw);
		} catch {
	...
	if (proxy === "ag") {
		...
		switch (proxy) {
			...
			case "rh":
				return await RammerheadEncode(raw);
			default:
				return __uv$config.prefix + __uv$config.encodeUrl(raw);
		}
	}
