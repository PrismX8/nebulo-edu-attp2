document.addEventListener('DOMContentLoaded', () => {
    ...
    function broadcastSettingChange(key, value) {
        try {
            const msg = { type: 'setting-changed', key, value };
            // If settings is inside the Nebulo tab iframe, notify the parent UI.
            if (window.parent && window.parent !== window) window.parent.postMessage(msg, location.origin);
            // Also notify top if nested.
            if (window.top && window.top !== window) window.top.postMessage(msg, location.origin);
        } catch {}
    }
    ...
    const loadSelections = () => {
        ...
        // Treat these as opt-in, not opt-out.
        if (abCheckbox) abCheckbox.checked = localStorage.getItem('ab') === 'true';
        if (clickoffCheckbox) clickoffCheckbox.checked = localStorage.getItem('clickoff') === 'true' || localStorage.getItem('clickoff') === null;
