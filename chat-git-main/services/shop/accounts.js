...
async function createAccount(options = {}) {
  try {
    // Generate pseudo-email/username if not provided
    const email = options.email || `user${Date.now()}@kchat.local`;
    const baseUsername = String(options.username || email.split('@')[0] || `user${Date.now()}`)
      .trim()
    ...
    // Create user in local JSON auth store
...
async function validateLogin(email, password) {
  try {
    ...
    const user = userStore.findByIdentifier(email);
