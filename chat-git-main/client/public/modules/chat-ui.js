...
export function createChatUi(deps) {
  ...
  const getCurrentVoiceChannel = () => {
    const textRoom = String(state.currentChannel?.room || 'global').trim().toLowerCase();
    const textName = String(state.currentChannel?.name || 'Current Room').replace(/^#|^@/, '').trim();
    return {
      id: `voice:${textRoom}`,
      room: `voice:${textRoom}`,
      name: `${textName || 'Room'} Voice`
    };
  };
  const getVoiceChannelLabel = (roomName = '') => {
    const currentVoice = getCurrentVoiceChannel();
    if (String(roomName || '').trim() === currentVoice.room) return currentVoice.name;
  ...
  const updateVoiceButtonLabel = (callInfo = state.activeVoiceCall) => {
    ...
    const isJoined = !!state.activeVoiceCall;
    const hasActiveCall = Array.isArray(callInfo?.participants) && callInfo.participants.length > 0;
    const text = isJoined ? 'Leave Voice' : hasActiveCall ? 'Join Voice' : 'Voice';
    if (label) label.textContent = text;
    voiceBtn.title = isJoined ? `Leave ${getVoiceChannelLabel(state.activeVoiceCall?.roomName)}` : `Join ${getCurrentVoiceChannel().name}`;
  ...
  const refreshVoiceCalls = async () => {
    try {
      const data = await voiceApi.getActiveCalls();
      const calls = Array.isArray(data?.calls) ? data.calls : [];
      state.voiceCallsByRoom = {};
      calls.forEach((call) => {
        const room = String(call?.roomName || '').trim();
        if (room) state.voiceCallsByRoom[room] = call;
      });
  ...
  const joinOrToggleVoiceRoom = async (voiceChannel = getCurrentVoiceChannel()) => {
    ...
    try {
      ...
      if (call) {
        ...
        const startResponse = await voiceApi.startVoiceCall(roomName, 'voice', channel.name || getVoiceChannelLabel(roomName), participantName);
  ...
  function renderMessages() {
    ...
    root.innerHTML = visibleMessages.map((m) => {
      ...
      const isPending = !!m?.pending || id.startsWith('temp-');
      const mine = isMine(m);
      ...
      return `
        ${unreadMarker}
        <div class="msg ${mine ? 'mine' : ''} ${isDeleted ? 'deleted' : ''} ${isPending ? 'pending' : ''} ${isSystem ? `system-note system-${systemKind}` : ''}${enterClass}" data-message-id="${esc(id)}" data-message-token="${esc(token)}"${enterStyle}>
          ...
          <div class="msg-body">
            <div class="msg-head">
              ...
              ${isPending ? '<span class="msg-send-state">Sending...</span>' : ''}
            </div>
  ...
  const upsertRealtimeMessage = (message, { replaceOptimistic = false } = {}) => {
    ...
    const messageId = getMessageId(message);
    const cleanMessage = replaceOptimistic ? { ...message, pending: false } : message;
    ...
    if (replaceOptimistic && optimisticIndex >= 0) {
      state.messages.splice(optimisticIndex, 1, cleanMessage);
      if (messageId) {
    ...
    state.messagesPromise = (async () => {
      ...
      if (changed) {
        ...
        if (nextMessages.length === 0) {
          ...
          state.messages = nextMessages.filter((message) => {
            const messageId = getMessageId(message);
  ...
  const layoutShell = (contentHtml, footerHtml = '') => {
    ...
    document.getElementById('sidebar')?.addEventListener('click', (event) => {
      if (event.target.closest('.channel-item') || event.target.closest('#btn-add-friend')) {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const canonicalId = String(sent?.id || sent?._id || '').trim();
      if (canonicalId) {
        realtimeMessage.id = canonicalId;
        realtimeMessage._id = canonicalId;
      }
  ...
  const renderChatPage = async (channelId) => {
    ...
    const previousVoiceRoomId = previousRoomId ? `voice:${previousRoomId.toLowerCase()}` : '';
    if (
      previousRoomId &&
      nextRoomId &&
    ...
    if (voiceChatBtn) {
      voiceChatBtn.style.display = '';
      voiceChatBtn.classList.toggle('active', !!state.activeVoiceCall);
      const currentVoice = getCurrentVoiceChannel();
      updateVoiceButtonLabel(state.activeVoiceCall || state.voiceCallsByRoom?.[currentVoice.room] || null);
    ...
    state.pollTimer = setInterval(async () => {
      if (!state.currentChannel) return;
      const now = Date.now();
      if (document.hidden && now - Number(state.lastHiddenMessagePollAt || 0) < 2500) return;
      if (document.hidden) state.lastHiddenMessagePollAt = now;
      ...
      try {
        ...
        state.messagePollFailures = Number(state.messagePollFailures || 0) + 1;
        if (state.messagePollFailures >= 3) {
          try {
            await joinRoom(state.currentChannel);
            await getMessages(state.currentChannel, false, { live: true });
            state.messagePollFailures = 0;
            ...
            if (retryErr.status === 403) {
              state.bannedMessage = retryErr.message;
              renderMessages();
            }
          }
        }
